  ...
  const boxScore = data.table.rows
    ...
    .map((array) => {
      ...
      playerObject.three_percentage = isNaN(
        ...
        ? '0.00%'
        : ((playerObject.three_made / playerObject.three_attempts) * 100).toFixed(2).toString() +
          '%';
...
export const calculateTotals = (data) => {
  ...
  return teams.map((team) => {
    ...
    team.forEach((player) => {
      totalStats.fg_made += typeof player.fg_made === 'number' ? player.fg_made : 0;
      totalStats.fg_attempts += typeof player.fg_attempts === 'number' ? player.fg_attempts : 0;
      totalStats.three_made += typeof player.three_made === 'number' ? player.three_made : 0;
      totalStats.three_attempts +=
        typeof player.three_attempts === 'number' ? player.three_attempts : 0;
      totalStats.ft_made += typeof player.ft_made === 'number' ? player.ft_made : 0;
      totalStats.ft_attempts += typeof player.ft_attempts === 'number' ? player.ft_attempts : 0;
      totalStats.points += typeof player.points === 'number' ? player.points : 0;
